...
export const registerEmployer = async (req: Request, res: Response) => {
	const safe = ValidationSchema.registerEmployer.safeParse(req.fields);
	if (!safe.success) throw new ValidationError(safe.error);

	const safeFiles = ValidationSchema.registerEmployerFile.safeParse(req.files);
	if (!safeFiles.success) throw new ValidationError(safeFiles.error);
	...
	const company_logo = (uploadAvFileRes as UploadApiResponse).url;
	...
	const user = await db.user.create({
		data: {
			...
			username,
			is_applicant: false,
			employer_details: { create: { ...employer_details, company_logo } },
		},
	});
