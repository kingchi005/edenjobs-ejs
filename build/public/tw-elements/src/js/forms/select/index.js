...
const selection_model_1 = __importDefault(require("./selection-model"));
...
class Select {
    constructor(element, config, classes) {
        ...
        this._selectionModel = new selection_model_1.default(this.multiple);
        this._activeOptionIndex = -1;
        this._activeOption = null;
        this._wrapperId = (0, index_1.getUID)("select-wrapper-");
        this._dropdownContainerId = (0, index_1.getUID)("select-dropdown-container-");
        this._selectAllId = (0, index_1.getUID)("select-all-");
        this._debounceTimeoutId = null;
        this._dropdownHeight =
            this._config.selectOptionHeight * this._config.selectVisibleOptions;
        this._popper = null;
        this._input = null;
        this._label = selector_engine_1.default.next(this._element, SELECTOR_LABEL)[0];
        this._notch = null;
        this._fakeValue = null;
        this._isFakeValueActive = false;
        this._customContent = selector_engine_1.default.next(element, SELECTOR_CUSTOM_CONTENT)[0];
    ...
    get filterInput() {
        return selector_engine_1.default.findOne(SELECTOR_FILTER_INPUT, this._dropdownContainer);
    }
    get dropdown() {
        return selector_engine_1.default.findOne(SELECTOR_DROPDOWN, this._dropdownContainer);
    }
    get optionsList() {
        return selector_engine_1.default.findOne(SELECTOR_OPTIONS_LIST, this._dropdownContainer);
    }
    get optionsWrapper() {
        return selector_engine_1.default.findOne(SELECTOR_OPTIONS_WRAPPER, this._dropdownContainer);
    }
    get clearButton() {
        return selector_engine_1.default.findOne(SELECTOR_CLEAR_BUTTON, this._wrapper);
    }
    get options() {
    ...
    _getPlainOptions(optionsToRender) {
        const hasOptionGroup = selector_engine_1.default.findOne("optgroup", this._element);
        if (!hasOptionGroup) {
    ...
    _createOptionObject(nativeOption, group = {}) {
        const id = (0, index_1.getUID)("option-");
        const groupId = group.id ? group.id : null;
        const groupDisabled = group.disabled ? group.disabled : false;
        const selected = nativeOption.selected || nativeOption.hasAttribute(DATA_SELECTED);
        ...
        const secondaryText = manipulator_1.default.getDataAttribute(nativeOption, "selectSecondaryText");
        const icon = manipulator_1.default.getDataAttribute(nativeOption, "select-icon");
        return new select_option_1.default(id, nativeOption, multiple, value, label, selected, disabled, hidden, secondaryText, groupId, icon);
    }
    ...
    _init() {
        ...
        this._wrapper = selector_engine_1.default.findOne(`#${this._wrapperId}`);
        this._input = selector_engine_1.default.findOne(SELECTOR_INPUT, this._wrapper);
        ...
        if (containerSelector === "body") {
            ...
            this._container = selector_engine_1.default.findOne(containerSelector);
        }
        ...
        this._dropdownContainer = (0, templates_1.getDropdownTemplate)(this._dropdownContainerId, this._config, this._input.offsetWidth, this._dropdownHeight, this._selectAllOption, this._optionsToRender, this._customContent, this._classes);
        this._setFirstActiveOption();
        this._listenToFocusChange();
    }
    _renderMaterialWrapper() {
        const template = (0, templates_1.getWrapperTemplate)(this._wrapperId, this._config, this._label, this._classes, this._config.customArrow);
        this._element.parentNode.insertBefore(template, this._element);
        manipulator_1.default.addClass(this._element, this._classes.initialized);
        template.appendChild(this._element);
    }
    _initOutlineInput() {
        const inputWrapper = selector_engine_1.default.findOne(SELECTOR_FORM_OUTLINE, this._wrapper);
        const outlineInput = new input_1.default(inputWrapper, {
            inputFormWhite: this._config.selectFormWhite,
        }, this._classes);
        outlineInput.init();
        this._notch = selector_engine_1.default.findOne(SELECTOR_NOTCH, this._wrapper);
    }
    ...
    _listenToComponentKeydown() {
        event_handler_1.default.on(this._wrapper, "keydown", this._handleKeydown.bind(this));
    }
    ...
    _handleOpenKeydown(event) {
        ...
        if (key === keycodes_1.TAB && this._config.selectAutoSelect && !this.multiple) {
    ...
    _handleClosedKeydown(event) {
        const key = event.keyCode;
        if (key === keycodes_1.ENTER) {
            event.preventDefault();
        }
        const isOpenKey = key === keycodes_1.ENTER ||
            (key === keycodes_1.DOWN_ARROW && event.altKey) ||
            (key === keycodes_1.DOWN_ARROW && this.multiple);
        if (isOpenKey) {
            this.open();
        }
        if (!this.multiple) {
    ...
    _listenToWrapperClick() {
        event_handler_1.default.on(this._wrapper, "click", () => {
            this.toggle();
        });
    }
    _listenToClearBtnClick() {
        event_handler_1.default.on(this.clearButton, "click", (event) => {
            event.preventDefault();
    ...
    _listenToClearBtnKeydown() {
        event_handler_1.default.on(this.clearButton, "keydown", (event) => {
            if (event.keyCode === keycodes_1.ENTER) {
                this._handleClear();
                event.preventDefault();
                event.stopPropagation();
            }
    ...
    _listenToOptionsClick() {
        event_handler_1.default.on(this.optionsWrapper, "click", (event) => {
            const optionGroupLabel = event.target.hasAttribute(DATA_SELECT_OPTION_GROUP_LABEL);
            if (optionGroupLabel) {
                return;
            }
            const target = event.target.nodeName === "DIV"
                ? event.target
                : selector_engine_1.default.closest(event.target, SELECTOR_OPTION);
    ...
    _singleOptionSelect(option) {
        ...
        if (currentSelected && currentSelected !== option) {
            ...
            event_handler_1.default.trigger(this._element, EVENT_DESELECT, {
    ...
    _emitValueChangeEvent(value) {
        event_handler_1.default.trigger(this._element, EVENT_VALUE_CHANGE, { value });
    }
    _emitNativeChangeEvent() {
        event_handler_1.default.trigger(this._element, EVENT_CHANGE);
    }
    _updateInputValue() {
    ...
    _appendFakeValue() {
        ...
        this._fakeValue = (0, templates_1.getFakeValueTemplate)(value, this._classes);
        const inputWrapper = selector_engine_1.default.findOne(SELECTOR_FORM_OUTLINE, this._wrapper);
    ...
    _updateClearButtonVisibility() {
        ...
        const hasSelection = this._selectionModel.selection ||
            this._selectionModel.selections.length > 0;
        if (hasSelection) {
            manipulator_1.default.addStyle(this.clearButton, { display: "block" });
        }
        else {
            manipulator_1.default.addStyle(this.clearButton, { display: "none" });
        }
    }
    _updateSelectAllState() {
        const selectAllSelected = this._selectAllOption.selected;
        const allSelected = (0, util_1.default)(this.options);
    ...
    _openDropdown() {
        this._popper = (0, core_1.createPopper)(this._input, this._dropdownContainer, {
    ...
    _setInputActiveStyles() {
        ...
        selector_engine_1.default.findOne(SELECTOR_NOTCH, this._wrapper).setAttribute(DATA_FOCUSED, "");
    }
    ...
    _filterOptions(searchTerm) {
        ...
        this._optionsToRender.forEach((option) => {
            const isOptionGroup = Object.prototype.hasOwnProperty.call(option, "options");
            const isValidOption = !isOptionGroup &&
    ...
    _updateOptionsListTemplate(optionsToRender) {
        const optionsWrapperContent = selector_engine_1.default.findOne(SELECTOR_OPTIONS_LIST, this._dropdownContainer) ||
            selector_engine_1.default.findOne(SELECTOR_NO_RESULTS, this._dropdownContainer);
        const optionsListTemplate = (0, templates_1.getOptionsListTemplate)(optionsToRender, this._selectAllOption, this._config, this._classes);
    ...
    _listenToDropdownKeydown() {
        event_handler_1.default.on(this.dropdown, "keydown", this._handleOpenKeydown.bind(this));
    }
    _listenToOutsideClick() {
        this._outsideClick = this._handleOutSideClick.bind(this);
        event_handler_1.default.on(document, "click", this._outsideClick);
    }
    _listenToFocusChange(add = true) {
        if (add === false) {
            event_handler_1.default.off(this._input, "focus", () => this._notch.setAttribute(DATA_FOCUSED, ""));
            event_handler_1.default.off(this._input, "blur", () => this._notch.removeAttribute(DATA_FOCUSED));
            return;
        }
        event_handler_1.default.on(this._input, "focus", () => this._notch.setAttribute(DATA_FOCUSED, ""));
        event_handler_1.default.on(this._input, "blur", () => this._notch.removeAttribute(DATA_FOCUSED));
    }
    _handleOutSideClick(event) {
        ...
        if (!this._toggleButton) {
            this._elementToggle = selector_engine_1.default.find(SELECTOR_TOGGLE);
        }
        if (this._elementToggle) {
            this._elementToggle.forEach((button) => {
                const attributes = manipulator_1.default.getDataAttribute(button, "select-toggle");
                if (attributes === this._element.id ||
    ...
    close() {
        ...
        const transitionTime = (0, index_1.getTransitionDurationFromElement)(this._dropdownContainer.children[0]);
        ...
        setTimeout(() => {
            ...
            this._input.blur();
            selector_engine_1.default.findOne(SELECTOR_NOTCH, this._wrapper).removeAttribute(DATA_FOCUSED);
    ...
    _removeDropdownEvents() {
        event_handler_1.default.off(document, "click", this._outsideClick);
        if (this._config.selectFilter) {
            event_handler_1.default.off(this.dropdown, "keydown");
        }
        event_handler_1.default.off(this.optionsWrapper, "click");
    }
    ...
    _updateDisabledState() {
        const input = selector_engine_1.default.findOne(SELECTOR_INPUT, this._wrapper);
        if (this._element.hasAttribute("disabled")) {
    ...
    _createSelectAllOption() {
        ...
        const selected = (0, util_1.default)(this.options);
    ...
    _removeComponentEvents() {
        event_handler_1.default.off(this.input, "click");
        event_handler_1.default.off(this.wrapper, this._handleKeydown.bind(this));
        event_handler_1.default.off(this.clearButton, "click");
        event_handler_1.default.off(this.clearButton, "keydown");
    ...
    _destroyMaterialTemplate() {
        ...
        const labels = selector_engine_1.default.find("label", this._wrapper);
        ...
        manipulator_1.default.removeClass(this._element, this._classes.initialized);
