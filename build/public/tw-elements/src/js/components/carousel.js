...
const SELECTOR_DATA_NEXT_PREV = "[data-te-carousel-item-next], [data-te-carousel-item-prev]";
...
class Carousel extends base_component_1.default {
    constructor(element, config, classes) {
        ...
        this._indicatorsElement = selector_engine_1.default.findOne(SELECTOR_DATA_INDICATORS, this._element);
    ...
    pause(event) {
        ...
        if (selector_engine_1.default.findOne(SELECTOR_DATA_NEXT_PREV, this._element)) {
    ...
    to(index) {
        this._activeElement = selector_engine_1.default.findOne(SELECTOR_DATA_ACTIVE_ITEM, this._element);
    ...
    _init() {
        ...
        event_handler_1.default.on(window, EVENT_LOAD_DATA_API, () => {
            const carousels = selector_engine_1.default.find(SELECTOR_DATA_CAROUSEL_INIT);
    ...
    _enableCycle() {
        ...
        if (this._isSliding) {
            event_handler_1.default.one(this._element, EVENT_SLID, () => this.cycle());
            return;
        }
        this.cycle();
    }
    _applyInitialClasses() {
        const activeElement = selector_engine_1.default.findOne(SELECTOR_DATA_ACTIVE_ITEM, this._element);
        activeElement.classList.add(this._classes.block, ...this._classes.visible.split(" "));
    ...
    _setActiveElementClass() {
        this._activeElement = selector_engine_1.default.findOne(SELECTOR_DATA_ACTIVE_ITEM, this._element);
        manipulator_1.default.addClass(this._activeElement, "hidden");
    }
    _addEventListeners() {
        ...
        if (this._config.pause === "hover") {
            ...
            event_handler_1.default.on(this._element, EVENT_MOUSELEAVE, (event) => this._enableCycle(event));
        }
    ...
    _addTouchEventListeners() {
        ...
        const end = (event) => {
            ...
            if (this._config.pause === "hover") {
                ...
                this.touchTimeout = setTimeout((event) => this._enableCycle(event), TOUCHEVENT_COMPAT_WAIT + this._config.interval);
            }
        };
        selector_engine_1.default.find(SELECTOR_DATA_ITEM_IMG, this._element).forEach((itemImg) => {
            event_handler_1.default.on(itemImg, EVENT_DRAG_START, (event) => event.preventDefault());
        });
    ...
    _getItemIndex(element) {
        this._items =
            element && element.parentNode
                ? selector_engine_1.default.find(SELECTOR_DATA_ITEM, element.parentNode)
                : [];
    ...
    _triggerSlideEvent(relatedTarget, eventDirectionName) {
        ...
        const fromIndex = this._getItemIndex(selector_engine_1.default.findOne(SELECTOR_DATA_ACTIVE_ITEM, this._element));
    ...
    _setActiveIndicatorElement(element) {
        if (this._indicatorsElement) {
            const activeIndicator = selector_engine_1.default.findOne(SELECTOR_DATA_ACTIVE, this._indicatorsElement);
            ...
            for (let i = 0; i < indicators.length; i++) {
                if (Number.parseInt(indicators[i].getAttribute("data-te-slide-to"), 10) === this._getItemIndex(element)) {
    ...
    _updateInterval() {
        const element = this._activeElement ||
            selector_engine_1.default.findOne(SELECTOR_DATA_ACTIVE_ITEM, this._element);
        if (!element) {
            return;
        }
        const elementInterval = Number.parseInt(element.getAttribute("data-te-interval"), 10);
        if (elementInterval) {
    ...
    _slide(directionOrOrder, element) {
        ...
        const activeClass = directionalAttr === ATTR_START
            ? this._classes.slideLeft
            : this._classes.slideRight;
        const nextClass = directionalAttr !== ATTR_START
            ? this._classes.slideLeft
            : this._classes.slideRight;
        ...
        if (this._element.hasAttribute(ATTR_CAROUSEL)) {
            ...
            activeElement.classList.add(activeClass, ...this._classes.invisible.split(" "));
            ...
            nextElement.classList.remove(this._classes.slideRight, this._classes.slideLeft);
            const completeCallBack = () => {
                ...
                activeElement.classList.remove(activeClass, ...this._classes.invisible.split(" "), this._classes.block);
