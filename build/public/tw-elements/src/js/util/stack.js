...
class Stack {
    constructor(element, selector, options) {
        ...
        if (this._options.container) {
            this._parent = selector_engine_1.default.findOne(this._options.container);
        }
    }
    get stackableElements() {
        return selector_engine_1.default.find(this._selector)
            .filter((el, i) => this._options.filter(el, i))
            .map((el) => ({ el, rect: el.getBoundingClientRect() }))
            .filter(({ el, rect }) => {
            const basicCondition = el !== this._element && (0, index_1.isVisible)(el);
