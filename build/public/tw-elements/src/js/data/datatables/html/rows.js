...
const rows = ({ rows, columns, noFoundMessage, edit, selectable, loading, bordered, borderless, striped, hover, sm, classes, }) => {
    const rowsTemplate = rows.map((row) => {
        const checkbox = `
      <td data-te-field="checkbox" class="${bordered ? `${classes.tableBordered} ${classes.borderColor}` : ""}">
        ...
        const innerRow = columns
            ...
            const cssText = Object.keys(style)
                ...
                .join("; ");
            return `<td style="${cssText}" class="${classes.rowItem} ${classes.borderColor} ${edit ? `${classes.edit}` : ""} ${bordered ? `${classes.tableBordered}` : ""} ${sm ? `${classes.sm}` : ""} ${column.fixed ? `${classes.fixedHeader} ${classes.color}` : ""}" ${ATTR_CELL} data-te-field="${column.field}" ${edit && 'contenteditable="true"'}>${row[column.field]}</td>`;
        })
            .join("");
        return `<tr scope="row" class="${classes.row} ${classes.borderColor} ${classes.rowAnimation} ${striped ? `${classes.striped}` : ""} ${borderless ? `${classes.borderless}` : ""} ${hover ? `${classes.hoverRow}` : ""}" data-te-index="${row.rowIndex}" ${ATTR_ROW}>${selectable ? checkbox : ""}${innerRow}</tr>`;
    });
