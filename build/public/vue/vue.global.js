...
var Vue = (function (exports) {
    ...
    const NOOP = () => { };
    const NO = () => false;
    const isOn = (key) => key.charCodeAt(0) === 111 &&
    ...
    const isBooleanAttr = makeMap(specialBooleanAttrs +
    ...
    function renderComponentRoot(instance) {
        ...
        try {
            if (vnode.shapeFlag & 4) {
                ...
                result = normalizeVNode(render2.length > 1
                    ? render2(props, true
                        ? {
                            get attrs() {
                                markAttrsAccessed();
...
If this is a native custom element, make sure to exclude it from component resolution via compilerOptions.isCustomElement.`
    ...
    function doWatch(source, cb, { immediate, deep, flush, once, onTrack, onTrigger } = EMPTY_OBJ) {
        ...
        const instance = getCurrentScope() === ((_a = currentInstance) == null ? void 0 : _a.scope)
            ? currentInstance
            : null;
        let getter;
    ...
    const createHook = (lifecycle) => (hook, target = currentInstance) => (!isInSSRComponentSetup || lifecycle === "sp") &&
    ...
    const PublicInstanceProxyHandlers = {
        get({ _: instance }, key) {
            ...
            if (key[0] !== "$") {
                ...
                else if ((normalizedProps = instance.propsOptions[0]) &&
...
If you want to remount the same app, move your app creation logic into a factory function and create fresh app instances for each mount - e.g. \`const createMyApp = () => createApp(App)\``);
    ...
    const isSVGContainer = (container) => container.namespaceURI.includes("svg") &&
...
Server rendered element contains more child nodes than client vdom.`);
  ...
  - expected on client: ${vnode.children}`);
                ...
                if ((vnodeHooks = props && props.onVnodeMounted) ||
                    dirs ||
                    needCallTransitionHooks) {
...
- rendered on server:`, node, node.nodeType === 3
                ? `(text)`
...
- expected on client:`, vnode.type);
    ...
    function propHasMismatch(el, key, clientValue) {
        ...
        else if ((el instanceof SVGElement && isKnownSvgAttr(key)) ||
    ...
    function baseCreateRenderer(options, createHydrationFns) {
                ...
                if (i <= e2) {
                    ...
                    while (i <= e2) {
                        patch(null, (c2[i] = optimized
                            ? cloneIfMounted(c2[i])
                            : normalizeVNode(c2[i])), container, anchor, parentComponent, parentSuspense, namespace, slotScopeIds, optimized);
                        i++;
                    }
                }
            }
        ...
        const unmount = (vnode, parentComponent, parentSuspense, doRemove = false, optimized = false) => {
            ...
            if (shapeFlag & 6) {
                ...
                else if ((type === Fragment && patchFlag & (128 | 256)) ||
...
Component that was made reactive: `, type);
    ...
    const isNativeOn = (key) => key.charCodeAt(0) === 111 &&
        key.charCodeAt(1) === 110 &&
        key.charCodeAt(2) > 96 &&
                ...
                if (props && !isArray(props)) {
                    for (const key in props) {
                        ...
                        if (opt === Number || (opt && opt.type === Number)) {
                            ...
                            (numberProps ||
                                (numberProps = Object.create(null)))[camelize(key)] = true;
                        }
                    }
                }
    ...
    function setSelected(el, value) {
        ...
        if (isMultiple && !isArray(value) && !isSet(value)) {
            warn(`<select multiple v-model> expects an Array or Set value for its binding, but got ${Object.prototype.toString
                .call(value)
                .slice(8, -1)}.`);
            return;
        }
    ...
    function resolveRootNamespace(container) {
        ...
        if (typeof MathMLElement === "function" &&
    ...
    function parseForExpression(input) {
        ...
        if (iteratorMatch) {
            ...
            if (iteratorMatch[2]) {
                ...
                if (indexContent) {
                    result.index = createAliasExpression(indexContent, exp.indexOf(indexContent, result.key
                        ? keyOffset + keyContent.length
                        : trimmedOffset + valueContent.length), true);
                }
            }
        }
        if (valueContent) {
...
`, -1);
    ...
    function createCodegenNodeForBranch(branch, keyIndex, context) {
        if (branch.condition) {
            return createConditionalExpression(branch.condition, createChildrenCodegenNode(branch, keyIndex, context), createCallExpression(context.helper(CREATE_COMMENT), ['"v-if"', "true"]));
        }
        else {
    ...
    const transformElement = (node, context) => {
        return function postTransformElement() {
            ...
            let vnodeDirectives;
            let shouldUseBlock = isDynamicComponent ||
                vnodeTag === TELEPORT ||
                vnodeTag === SUSPENSE ||
                (!isComponent &&
    ...
    const transformOn$1 = (dir, node, context, augmentor) => {
        ...
        if (arg.type === 4) {
            if (arg.isStatic) {
                ...
                const eventString = node.tagType !== 0 ||
